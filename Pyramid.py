...
class EnhancedPositionalEncodingLayer(nn.Module):
    def __init__(self, in_channels, dim=6, norm=nn.LayerNorm):
        super().__init__()
        self.norm = norm(dim)
        self.proj = nn.Linear(in_channels, dim)
        ...
        self.feature_weight = nn.Parameter(torch.tensor(0.5))


        self.proj.weight = nn.Parameter(Normal(0, 1e-5).sample(self.proj.weight.shape))
        self.proj.bias = nn.Parameter(torch.zeros(self.proj.bias.shape))

    def forward(self, high_feat, raw_feat):


        w = torch.sigmoid(self.feature_weight)


        fused_feat = high_feat * w + raw_feat * (1 - w)


        fused_feat = fused_feat.permute(0, 2, 3, 4, 1)
        fused_feat = self.norm(self.proj(fused_feat))
        return fused_feat
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, size, mode='bilinear'):
        ...
        grid = torch.stack(grids) # [3, H, W, D]
        grid = torch.unsqueeze(grid, 0) # [1, 3, H, W, D]
    ...
    def forward(self, src, flow):
        ...
        # Move channels to last dimension: [B, H, W, D, 3]
        if len(shape) == 2:
...
class VecInt(nn.Module):
    """
    Integrates a stationary velocity field (SVF) into a deformation field
    """
...
class DCAP(nn.Module):
    def __init__(self,
        ...
        self.peblock1 = EnhancedPositionalEncodingLayer(2 * c , dim=head_dim*num_heads[4])
        self.flow_peblock1 = PositionalEncodingLayer(3, dim=head_dim * num_heads[4] * 27 // 2)
        self.lat1 = LAT(head_dim*num_heads[3], num_heads[4], qk_scale=scale)
        self.dfi1 = DFIBlock(3 * num_heads[4], 3 * 2 * num_heads[4])

        self.peblock2 = EnhancedPositionalEncodingLayer(4 * c , dim=head_dim * num_heads[3])
        self.flow_peblock2 = PositionalEncodingLayer(3, dim=head_dim * num_heads[3] * 27 // 2)
        self.lat2 = LAT(head_dim * num_heads[3], num_heads[3], qk_scale=scale)
        self.dfi2 = DFIBlock(3 * num_heads[3], 3 * 2 * num_heads[3])

        self.peblock3 = EnhancedPositionalEncodingLayer(8 * c , dim=head_dim * num_heads[2])
        self.flow_peblock3 = PositionalEncodingLayer(3, dim=head_dim * num_heads[2] * 27 // 2)
        self.lat3 = LAT(head_dim * num_heads[2], num_heads[2], qk_scale=scale)
        self.dfi3 = DFIBlock(3 * num_heads[2], 3 * num_heads[2] * 2)

        self.peblock4 = EnhancedPositionalEncodingLayer(16 * c , dim=head_dim * num_heads[1])
        self.flow_peblock4 = PositionalEncodingLayer(3, dim=head_dim * num_heads[1] * 27 // 2)
        self.lat4 = LAT(head_dim * num_heads[1], num_heads[1], qk_scale=scale)
        self.dfi4 = DFIBlock(3 * num_heads[1], 3 * num_heads[1] * 2)

        self.peblock5 = EnhancedPositionalEncodingLayer(32 * c , dim=head_dim * num_heads[0])
        self.lat5 = LAT(head_dim * num_heads[0], num_heads[0], qk_scale=scale)
        self.dfi5 = DFIBlock(3 * num_heads[0], 3 * num_heads[0] * 2)
    ...
    def forward(self, moving, fixed):
        ...
        q5 = self.peblock5(F5, f5)
        k5 = self.peblock5(M5, m5)

        w5 = self.lat5(q5, k5)
        w5 = self.dfi5(w5)
        flow5 = self.upsample_trilin(2 * w5)
        fields.append(flow5)


        warped_M4 = self.transformer[3](M4, flow5)
        warped_m4 = self.transformer[3](m4, flow5)

        q4 = self.peblock4(F4, f4)
        k4 = self.peblock4(warped_M4, warped_m4)

        w4 = self.lat4(q4, k4)
        w4 = self.dfi4(w4)
        flow4 = self.upsample_trilin(2 * (self.transformer[3](flow5, w4) + w4))  # 上采样到尺度3
        fields.append(flow4)

        warped_M3 = self.transformer[2](M3, flow4)
        warped_m3 = self.transformer[2](m3, flow4)

        q3 = self.peblock3(F3, f3)
        k3 = self.peblock3(warped_M3, warped_m3)

        w3, QQT, KKT = self.lat3(q3, k3, return_attn_matrix=True)
        w3 = self.dfi3(w3)
        flow3 = self.upsample_trilin(2 * (self.transformer[2](flow4, w3) + w3))
        fields.append(flow3)


        warped_M2 = self.transformer[1](M2, flow3)
        warped_m2 = self.transformer[1](m2, flow3)

        q2 = self.peblock2(F2, f2)
        k2 = self.peblock2(warped_M2, warped_m2)

        w2 = self.lat2(q2, k2)
        w2 = self.dfi2(w2)
        flow2 = self.upsample_trilin(2 * (self.transformer[1](flow3, w2) + w2))
        fields.append(flow2)

        warped_M1 = self.transformer[0](M1, flow2)
        warped_m1 = self.transformer[0](m1, flow2)

        q1 = self.peblock1(F1, f1)
        k1 = self.peblock1(warped_M1, warped_m1)

        w1 = self.lat1(q1, k1)
        w1 = self.dfi1(w1)
        flow1 = self.transformer[0](flow2, w1) + w1

        diff_flow = self.integrate(flow1)

        y_moved = self.transformer[0](moving, diff_flow)

        return y_moved, flow1, diff_flow